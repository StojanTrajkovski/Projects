    ...
    LikelihoodFieldModel::sensorFunction(nav2_amcl::LaserData * data, pf_sample_set_t * set)
    {
        ...
        for (j = 0; j < set->sample_count; j++) {
                ...
                */
                ...
                double x_s = pose_robot.v[0] + lidar_pose_to_robot.v[0]*cos(pose_robot.v[2]) - lidar_pose_to_robot.v[1]*sin(pose_robot.v[2]);
                double y_s = pose_robot.v[1] + lidar_pose_to_robot.v[0]*sin(pose_robot.v[2]) + lidar_pose_to_robot.v[1]*cos(pose_robot.v[2]);
                double theta_s = pose_robot.v[2] + lidar_pose_to_robot.v[2];
                ...
                for (i = 0; i < data->range_count; i += step) {
                    ...
                    */
                        ...
                        hit_x = x_s + obs_range*cos(theta_s + obs_bearing);
                        hit_y = y_s + obs_range*sin(theta_s + obs_bearing);

                        int m_x_hit = MAP_GXWX(self->map_, hit_x);
                        int m_y_hit = MAP_GYWY(self->map_, hit_y);

                        int index = MAP_INDEX(self->map_, m_x_hit, m_y_hit);

                        bool valid = MAP_VALID(self->map_, m_x_hit, m_y_hit);
