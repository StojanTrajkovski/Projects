...
namespace iar_astar_planner
{
    ...
    {
          // create cell arrays
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[NavFn] Constructor");
        costarr_ = NULL;
        potarr_ = NULL;
        pending_ = NULL;
        setNavArr(nx, ny);
    ...
    bool NavFn::propAstar(int cycles)
    {
        ...
        */
        ...
        float dist = std::hypot((start_[0] - goal_[0]), (start_[1] - goal_[1]));
        ...
        for(; cycle < cycles; cycle++)
        {
            ...
            */
            ...
            /* This is my version, but also have Tutorial 7 version below
            for (i = 0; i < n_curPotentArr_; i++) {
                int index = n_curPotentArr_[i];
            ...
            */
            ...
            if (n_curPotentArr_ == 0) {
                ...
                potentThresh_ = potentThresh_ + potentThreshInc_;   // Increment potential threshold, allowing higher costs to get into higher priority positions
            }
            ...
            */
            ...
            int goalIndex = goal_[1] * nx_ + goal_[0];
            if (potarr_[goalIndex] < POT_HIGHEST) {
                propSuccess = true;
                break;
            }
            ...
            */
                ...
                less than the existing saved potential estimate */ 
            {
                ...
                */
                ...
                int curX = n % nx_;
                int curY = n / nx_;
                float dist = std::hypot(goal_[0] - curX, goal_[1] - curY);
                pot = pot + dist;
    ...
    int NavFn::calcPath(int n)
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            if (potarr_[stc] < COST_NEUTRAL) {
                for(int i = 0;i<npath_;i++){
                    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"%f,%f",pathx_[i],pathy_[i]);
                }
                return npath_;  // done!
            }
            ...
            */
            ...
            float l = potarr_[stc - 1];
            float r = potarr_[stc + 1];
            float t = potarr_[stc - nx_];
            float b = potarr_[stc + nx_];
            float tl = potarr_[stc - nx_ - 1];
            float tr = potarr_[stc - nx_ + 1];
            float bl = potarr_[stc + nx_ - 1];
            float br = potarr_[stc + nx_ + 1];
            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"l%f r%f t%f  b%f tl%f tr%f bl%f br%f",l,r,t,b,tl,tr,bl,br);


            minp = std::min({l, r, t, b, tl, tr, bl, br});
            if (minp == l) {stc = stc - 1;}
            if (minp == r) {stc = stc + 1;}
            if (minp == t) {stc = stc - nx_;}
            if (minp == b) {stc = stc + nx_;}
            if (minp == tl) {stc = stc - nx_ - 1;}
            if (minp == tr) {stc = stc - nx_ + 1;}
            if (minp == bl) {stc = stc + nx_ - 1;}
            if (minp == br) {stc = stc + nx_ + 1;}

            minc = stc;

            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"idx:%d ([%d,%d]) pot:%d, minp:%f",stc,stc%nx_,stc/nx_,minp);
            if(minp >= curp)
            {
